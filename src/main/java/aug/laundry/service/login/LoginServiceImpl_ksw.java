...
public class LoginServiceImpl_ksw implements LoginService {
    ...
    public void naverLogin(HttpServletRequest request, Model model, HttpSession session) {
        ...
        try {
            ...
            Long checkRes = loginRepository.checkSocialId(response.get("id"));
            ...
            if(checkRes == null || checkRes <= 0L){
                ...
                memberDto.setAccount(response.get("email"));
                ...
                memberDto.setMemberMyInviteCode(memberRepository.getInviteCode());
                
                // DB에 등록
                Member saveMember = loginRepository.registerSocialUser(memberDto);
                loginRepository.registerSocialNumber(saveMember.getMemberId(), response.get("id"));

                // memberId를 받아 session에 저장하기
                Long memberId = loginRepository.socialLogin(memberAccount, "naver").getMemberId();
    ...
    public void kakaoLogin(String accessToken, KakaoOauthToken kOauthToken,HttpSession session){
        ...
        try {
            ...
            Long checkRes = loginRepository.checkSocialId(memberSocialId);

            if(checkRes <= 0){
                System.out.println("kakao 유저정보 db에 저장");
                // memberDto에 담기
                Member memberDto = new Member();
                memberDto.setAccount(kakaoProfile.getKakao_account().getEmail());
                ...
                Member saveMember = loginRepository.registerSocialUser(memberDto);
                loginRepository.registerSocialNumber(saveMember.getMemberId(), memberSocialId);

                // memberId를 가져와서 session에 저장하기
                Long memberId = loginRepository.socialLogin(kakaoProfile.getKakao_account().getEmail(), "kakao").getMemberId();
                session.setAttribute(SessionConstant.LOGIN_MEMBER, memberId);
    ...
    public Optional<? extends MemberParent> login(Member member) {
        if (member == null || member.getAccount() == null || member.getPassword() == null) return Optional.empty();

        Optional<? extends MemberParent> loginMember = memberRepository.findByMemberAccount(member.getAccount());

        if(loginMember.isPresent()){
            String encodedPassword = getPassword(loginMember.get());
            boolean isLogin = bc.matchBCrypt(member.getPassword(), encodedPassword);
            if(isLogin) return loginMember;
        }
    ...
    @Nullable
    private static String getPassword(MemberParent memberParent) {
        String encodedPassword = null;
        if (memberParent instanceof Member){
            encodedPassword = ((Member) memberParent).getPassword();
        } else if (memberParent instanceof Rider){
            encodedPassword = ((Rider) memberParent).getPassword();
        } else if (memberParent instanceof QuickRider){
            encodedPassword = ((QuickRider) memberParent).getPassword();
        } else if (memberParent instanceof Admin){
            encodedPassword = ((Admin) memberParent).getPassword();
        }
    ...
    public int keepLogin(String sessionId, Date limit, Long memberId) {
        int res = loginRepository.keepLogin(sessionId, limit, memberId);
        return res;
    }
    ...
    public int renewLoginTime(Long memberId) {
        return loginRepository.renewLoginTime(memberId);
    }
